...
def facility_guide_handler(state: ChatState) -> ChatState:
    ...
    intent_name = state.get("intent", "facility_guide") # 의도 이름을 명시적으로 지정하거나 state에서 가져옴
    ...
    vector_index_name = rag_config.get("vector_index_name") # config에서 인덱스 이름도 가져옵니다.
    ...
    try:
        ...
        retrieved_docs_text = perform_vector_search(
            ...
            vector_index_name=vector_index_name, # config에서 가져온 인덱스 이름 사용
