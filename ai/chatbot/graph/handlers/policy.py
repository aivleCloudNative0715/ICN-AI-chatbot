...
def arrival_policy_info_handler(state: ChatState) -> ChatState:
    """
    'arrival_policy_info' 의도에 대한 RAG 기반 핸들러.
    사용자 쿼리를 기반으로 MongoDB에서 입국 정책 정보를 검색하고 답변을 생성합니다.
    AirportPolicieVector 컬렉션을 사용합니다.
    """
    user_query = state.get("user_input", "")
    intent_name = state.get("intent", "arrival_policy_info")
...
def departure_policy_info_handler(state: ChatState) -> ChatState:
    """
    'departure_policy_info' 의도에 대한 RAG 기반 핸들러.
    사용자 쿼리를 기반으로 MongoDB에서 출국 정책 정보를 검색하고 답변을 생성합니다.
    AirportPolicieVector 컬렉션을 사용합니다.
    """
    user_query = state.get("user_input", "")
    intent_name = state.get("intent", "departure_policy_info")
