...
def llm_verify_intent_node(state: ChatState) -> ChatState:
    ...
    지침:
    1. **가장 중요한 지침:** 사용자의 마지막 질문이 날짜('오늘', '내일'), 요일, 또는 시간 정보만 포함하고 이전 대화가 'flight_info'와 관련 있다면, **최종 의도를 반드시 'flight_info'로 유지**하세요.
    2. '예측된 의도'가 완벽하게 일치하면, 최종 의도와 함께 재구성된 질문을 반환하세요.
    3. '예측된 의도'가 부적절하다고 판단되면, 사용 가능한 의도 목록에서 가장 적합한 의도를 찾아 새로운 의도와 함께 재구성된 질문을 반환하세요.
    4. 어떤 의도에도 해당되지 않으면, 그대로 예측된 의도와 재구성된 질문을 반환하세요.
    5. 절대 다른 설명이나 문장은 추가하지 말고, 오직 JSON 객체만 반환하세요.
    6. 재구성된 질문에는 이전 대화에서 언급된 중요한 정보(예: '미국행 비행기')를 반드시 포함하세요.
