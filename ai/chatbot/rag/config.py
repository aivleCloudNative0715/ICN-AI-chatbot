...
LLM_PROMPT_TEMPLATES = {
    ...
    3. 사용자 질문에 요금 계산이 필요할 경우, 다음 절차를 따르세요:
        a. 먼저 최초 요금이 부과되는 시간을 확인하고, 남은 시간을 계산하세요.
        b. 남은 시간을 추가 요금 단위 시간(예: 15분)으로 나누어 몇 번의 추가 요금이 필요한지 계산하세요.
        c. 각 요금을 합산하여 최종 금액을 도출하세요.
    ...
    2.  답변은 명확하고 간결하며, 사용자 질문에 해당하는 내용에 집중하세요.
    3.  시설/업체의 위치, 운영 시간, 주요 서비스 등을 포함하여 구체적으로 안내해주세요.
    4.  검색된 정보에 사용자 질문에 대한 내용이 없거나, 불충분할 경우, 답변할 수 없음을 명확히 알리세요. (예: "제가 가진 정보로는 해당 내용을 확인할 수 없습니다.")
