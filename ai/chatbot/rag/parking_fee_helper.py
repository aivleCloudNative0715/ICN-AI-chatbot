...
def _parse_parking_fee_query_with_llm(user_query: str) -> dict | None:
    """
    LLM을 사용하여 복합 주차 요금 질문을 개별 요청으로 분해하는 함수.
    """
    prompt_content = (
        "사용자 쿼리에서 주차 요금 및 할인과 관련된 개별 요청을 JSON 형식으로 추출해줘."
        ...
        "```"
        "\n\n예시: "
        "사용자: 제1터미널 주차 요금은 얼마고 2터미널은 주차비가 어때?"
        "응답: ```json\n{\"requests\": [{\"query\": \"제1터미널 주차 요금\"}, {\"query\": \"제2터미널 주차비\"}]}```"
        "사용자: 장기 주차 할인 되나요?"
        "응답: ```json\n{\"requests\": [{\"query\": \"장기 주차 할인\"}]}```"
    )
