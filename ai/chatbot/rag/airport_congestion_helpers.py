...
def _parse_query_with_llm(user_query: str) -> dict | None:
    ...
    prompt_content = (
        ...
        "지침: "
        "- **'지금', '현재'와 같은 표현은 특정 시간대 혼잡도를 묻는 것으로 간주하고 'is_daily'를 false로 설정해줘. 이 경우 'time'은 null로 추출해줘.**"
        "- **'하루 전체', '오늘 총', '하루'와 같은 표현이 있을 때만 'is_daily'를 true로 설정하고 'time'은 '합계'라는 문자열로 설정해줘.**"
