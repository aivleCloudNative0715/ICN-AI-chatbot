...
def fetch_and_upload_flight_schedule_data(db, airline_map, api_url, direction_kr, missing_airlines_set):
    """
...
# 2. 도착 편 스케줄 데이터 업로드
fetch_and_upload_flight_schedule_data(db, airline_mapping_dict, FLIGHT_SCHEDULE_ARRIVAL_API_URL, '도착', missing_airlines_overall)

# 3. 출발 편 스케줄 데이터 업로드
fetch_and_upload_flight_schedule_data(db, airline_mapping_dict, FLIGHT_SCHEDULE_DEPARTURE_API_URL, '출발', missing_airlines_overall)
...
if missing_airlines_overall:
    ...
    # 출력 폴더 생성 (없으면)
    output_directory = "./DB/MongoDB/Airline/output"
