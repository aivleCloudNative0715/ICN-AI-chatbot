...
try:
    ...
    db = client["AirBot"]
    airport_collection = db["TransitPath"] 
    airport_vectors_collection = db["TransitPathVector"]
    ...
    for doc in tqdm(documents_cursor, total=total_documents, desc="Processing Airports"):
        try:
            ...
            path_description = doc.get('path_description', '')

            # 의미를 잘 전달할 수 있는 문장 구성
            text_to_embed = f"{origin_terminal}에서 {destination_terminal}로 환승하는 경로는 다음과 같습니다. {path_description}"
