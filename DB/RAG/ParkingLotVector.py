...
try:
    ...
    db = client["AirBot"]
    airport_collection = db["ParkingLot"] 
    airport_vectors_collection = db["ParkingLotVector"]
    ...
    for doc in tqdm(documents_cursor, total=total_documents, desc="Processing Airports"):
        try:
            # 3. 임베딩할 텍스트 구성
            parking_type = doc.get('parking_type', '')
            ...
            text_to_embed = f"{terminal}에는 {floor} {zone} {parking_type}이 있습니다"
            
            text_to_embed = text_to_embed.replace('None', '').replace('  ', ' ').strip()
