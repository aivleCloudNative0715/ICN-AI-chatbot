# tests/test_departure_policy.py
...
# 챗봇의 핵심 모듈 임포트
# departure_policy_info_handler가 ai/chatbot/graph/handlers/policy.py에 있다고 가정합니다.
# 만약 다른 파일에 있다면 해당 파일로 경로를 수정해주세요.
from ai.chatbot.graph.handlers.policy import departure_policy_info_handler
...
def run_test_and_save_results_direct_handler(input_file_path: str, output_file_path: str):
    """
    입력 파일에서 쿼리를 읽어 'departure_policy_info_handler'를 직접 실행하고,
    결과를 출력 파일에 저장합니다.
    ...
    with open(output_file_path, 'w', encoding='utf-8') as f_out:
        f_out.write("--- 챗봇 출국 정책 정보 핸들러 직접 호출 테스트 결과 ---\n\n")
        ...
        for i, query in enumerate(user_queries):
            ...
            try:
                # ChatState 객체 생성 시, intent를 "departure_policy_info"로 강제 지정
                initial_state = ChatState(user_input=query, intent="departure_policy_info")
                
                # classify_intent 노드나 graph.invoke()를 사용하지 않고, 핸들러를 직접 호출
                final_state = departure_policy_info_handler(initial_state)
...
if __name__ == "__main__":
    ...
    input_queries_file = script_dir / "departure_queries.txt" # 출국 쿼리 파일명
    output_results_file = script_dir / "departure_test_results.txt" # 출국 결과 파일명
